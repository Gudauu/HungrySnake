...
{
	string show = "当前得分:";show.append(to_string(score));
...
void Puzzle::ReAlive()
{
	...
	SetText((NUM + XMARGIN /10) * DIA, DIA, "准备好，蛇将在3秒后复活...", 'p');
	Sleep(3000);
	...
	while (1)
	{
		...
		case 2:    //bonus
			...
			if (bonuschoice == 0)
			{
				...
				SetClear(Node(NUM, 0), XMARGIN * DIA, 2 * DIA, 'p');
				SetText((NUM + XMARGIN / 5) * DIA, DIA, "变颜色！", 'p');
				flushcount = 2000;
			}
			else {
				...
				if (bonuschoice == 1) {
					lives++;
					SetClear(Node(NUM, 0), XMARGIN * DIA, 2 * DIA, 'p');
					SetText((NUM + XMARGIN / 5) * DIA, DIA, "获得一条命！", 'p');
					flushcount = 2000;
				}
				else if (bonuschoice == 2) {
					speedscore %= 20;
					SetClear(Node(NUM, 0), XMARGIN * DIA, 2 * DIA, 'p');
					SetText((NUM + XMARGIN / 5) * DIA, DIA, "获得减速！", 'p');
					flushcount = 2000;
				}
			}
...
void Platform::Front()
{
	...
	outtextxy((NUM + XMARGIN / 3) * DIA-5, DIA, "操作指南");
	outtextxy((NUM + XMARGIN / 3) * DIA, 3*DIA, "上:w");
	outtextxy((NUM + XMARGIN / 3) * DIA, 5 * DIA, "下:s");
	outtextxy((NUM + XMARGIN / 3) * DIA, 7 * DIA, "左:a");
	outtextxy((NUM + XMARGIN / 3) * DIA, 9 * DIA, "右:d");
	outtextxy((NUM + XMARGIN / 9) * DIA-4, 11 * DIA, "准备好后，按任意键开始！");
	_getch();
	...
	puz.Play();
	setbkcolor(PLATFORM);
	outtextxy((NUM + XMARGIN / 3) * DIA - 5, DIA, "你挂了！");
	//closegraph();
	_getch();
}
